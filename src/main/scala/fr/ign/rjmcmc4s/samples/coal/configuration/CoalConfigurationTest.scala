...
object CoalConfigurationTest extends App {
  ...
  var c: Double = Math.min(1. / Math.min(1, KDistribution.pdf(1) / KDistribution.pdf(0)), 1.0 / Math.min(1, KDistribution.pdf(Kmax - 1) / KDistribution.pdf(Kmax)))
  for (k <- 1 to Kmax - 1)
    c = Math.min(c, 1. / Math.min(1., KDistribution.pdf(k + 1) / KDistribution.pdf(k)) + 1.0 / Math.min(1, KDistribution.pdf(k - 1) / KDistribution.pdf(k)))
  c = 0.9 * c
  println("c = " + c)
  def birthK(k: Int) = if (k == Kmax) 0. else c * Math.min(1., KDistribution.pdfRatio(k + 1, k))
  def deathK(k: Int) = if (k == 0) 0. else c * Math.min(1., KDistribution.pdfRatio(k - 1, k))
  ...
  val height: Configuration => Double = c => c match {
    case config: CoalConfiguration => if (config.K == 0) (1. - (birthK(config.K) + deathK(config.K))) else (1. - (birthK(config.K) + deathK(config.K))) / 2.
  }
  ...
  val position: Configuration => Double = c => c match {
    case config: CoalConfiguration => if (config.K == 0) 0. else height(c)
  }
